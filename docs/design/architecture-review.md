# Architecture review (ROB-84)

**Status: COMPLETE and tracked.** All five slices are done, §5 holds the triage,
and the release-blocking findings shipped in ROB-90 (5.1), ROB-91 (1.1/1.2) and
ROB-96 (4.1/4.2). Kept as the record of the review and of what it found.

Corrected 2026-09-20: this header previously read "IN PROGRESS. Deliberately
UNTRACKED … do not `git add` it", which was true while the review was running
and false afterwards — the file has been tracked since it was committed.

§0 below is the procedure the review ran under, kept because the method is the
reason the findings exist. If you are reading this cold, read §1 then §4.
...
Two more data points arrived after the issue was filed, both the same pattern —
...
not have caught cases Q and R."* Grep locates and counts; only reading supports
a conclusion.
...
| S4 | `action.cc` | Stage 1 complaint: "every mechanic lands in one file" | 1168+ lines — larger than `turn_loop.cc` (893) ever was. The god file moved rather than being broken up. | erosion |
...
ROB-87/88/89 had just changed every part of it. Freshness is not coverage.
